...
(function(){
  ...
  function scaleEnemy(base){
    ...
    return {
      ...
      atk:base.atk+(zone>0?Math.floor(zone*1.2):0),
      exp:zone>0?Math.max(1,Math.round(base.exp*(1+zone*.35))):base.exp,
      asleep:false,
      motion:'idle',
      facing:'s',
      respawned:true
  ...
  function spawnSpot(){
    ...
    for(let y=1;y<H-1;y++)for(let x=1;x<W-1;x++){
      ...
      if(game.chest&&!game.chest.open&&x===game.chest.x&&y===game.chest.y)continue;
      if(occupiedKeys.has(`${x},${y}`))continue;
      const d=Math.max(Math.abs(x-game.player.x),Math.abs(y-game.player.y));
  ...
  function tryRespawn(){
    ...
    if(game.floor===100)return;
    if((game.respawnCount||0)>=RESPAWN_LIMIT)return;
    if(game.turn-(game.lastRespawnCheck||0)<CHECK_EVERY)return;
    game.lastRespawnCheck=game.turn;
    const living=game.enemies.filter(e=>e.hp>0&&!e.isBoss).length;
    if(living>=MAX_LIVING||Math.random()>RESPAWN_CHANCE)return;
    const pool=floorPool();
    const p=spawnSpot();
    if(!pool.length||!p)return;
    const base=pool[rnd(pool.length)];
    const e=scaleEnemy(base);
    e.x=p.x;e.y=p.y;
    game.enemies.push(e);
    game.respawnCount=(game.respawnCount||0)+1;
    msg(`気配がする…… ${e.name}が現れた。`);
    log(`敵が再出現した：${e.name}（${game.respawnCount}/${RESPAWN_LIMIT}）`);
  }
  ...
  generateFloor=function(){
    ...
    game.respawnCount=0;
    game.lastRespawnCheck=game.turn||0;
    return r;
  };

  const baseEndTurn=endTurn;
  endTurn=function(){
    const r=baseEndTurn.apply(this,arguments);
    if(game&&!game.dead){tryRespawn();render();}
    return r;
  };
