...
(function(){
  ...
  function goblinStyle(dir){const col=ATLAS_COL[dir]??0;const x=(col/3*100).toFixed(4);return `background-image:url(${goblinAtlasSrc});background-size:400% 100%;background-position:${x}% 50%;background-repeat:no-repeat;background-origin:border-box;background-clip:border-box;`}
  ...
  async function loadGoblinAtlas(){goblinAtlasSrc=await loadTextAsset(GOBLIN_ATLAS_TEXT);document.querySelectorAll('.entity.enemy[title="緑小鬼"]').forEach(el=>delete el.dataset.fullSprite);refreshAll()}

  const obs=new MutationObserver(()=>requestAnimationFrame(refreshAll));
  window.addEventListener('DOMContentLoaded',()=>{const b=document.getElementById('board');if(b)obs.observe(b,{childList:true,subtree:true});refreshAll();loadGoblinAtlas();loadAtlas()});
})();