...
(function(){
  ...
  // Known-good 512 WebP already used successfully before the atlas experiments.
  // Use it for every facing temporarily so the goblin never falls back to '?' or legacy art.
  const GOBLIN_STABLE_TEXT='assets/goblin512_s.b64.txt';
  ...
  function refresh(el){
    const name=el.title,dir=facing(el),sig=name+'-'+dir+'-'+(atlasSrc?'a':'f')+'-'+(goblinStableSrc?'g':'p');
    if(el.dataset.fullSprite===sig)return;
    el.dataset.fullSprite=sig;
    const hp=el.querySelector('.enemyHp')?.outerHTML||'';
    if(name==='緑小鬼'){
      if(goblinStableSrc){
        el.innerHTML=`<span class="enemyGlyph directionalEnemy"><img class="enemySprite512 goblin512" src="${goblinStableSrc}" alt=""></span>${hp}`;
      }
      return;
    }
  ...
  async function loadTextAsset(url){try{const res=await fetch(url,{cache:'no-cache'});if(!res.ok)return '';const text=(await res.text()).trim();return text.startsWith('UklGR')?'data:image/webp;base64,'+text:''}catch(_e){return ''}}
  async function loadAtlas(){atlasSrc=await loadTextAsset(ATLAS_TEXT);document.querySelectorAll('.entity.enemy').forEach(el=>delete el.dataset.fullSprite);refreshAll()}
  async function loadGoblin(){goblinStableSrc=await loadTextAsset(GOBLIN_STABLE_TEXT);document.querySelectorAll('.entity.enemy[title="緑小鬼"]').forEach(el=>delete el.dataset.fullSprite);refreshAll()}
  const obs=new MutationObserver(()=>requestAnimationFrame(refreshAll));
  window.addEventListener('DOMContentLoaded',()=>{const b=document.getElementById('board');if(b)obs.observe(b,{childList:true,subtree:true});refreshAll();loadGoblin();loadAtlas()});
})();