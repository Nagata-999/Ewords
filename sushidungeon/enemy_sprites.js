...
(function(){
  ...
  function refresh(el){
    const name=el.title,dir=facing(el),sig=name+'-'+dir+'-'+(atlasSrc?'a':'f')+'-'+(goblinSrc[dir]?'g':'p');
    if(el.dataset.fullSprite===sig)return;
    el.dataset.fullSprite=sig;
    const hp=el.querySelector('.enemyHp')?.outerHTML||'';
    if(name==='緑小鬼'){
      const src=goblinSrc[dir]||goblinSrc.s;
      if(src) el.innerHTML=`<span class="enemyGlyph directionalEnemy"><img class="enemySprite512 goblin512" src="${src}" alt=""></span>${hp}`;
      else el.innerHTML=`<span class="enemyGlyph directionalEnemy goblin512" style="visibility:hidden"></span>${hp}`;
      return;
    }
  ...
  async function loadGoblins(){
    const dirs=['s','n','e','w'];
    const loaded=await Promise.all(dirs.map(d=>loadTextAsset(GOBLIN_TEXT[d])));
    dirs.forEach((d,i)=>{goblinSrc[d]=loaded[i]});
  ...
  window.addEventListener('DOMContentLoaded',()=>{const b=document.getElementById('board');if(b)obs.observe(b,{childList:true,subtree:true});refreshAll();loadGoblins();loadAtlas()});
})();