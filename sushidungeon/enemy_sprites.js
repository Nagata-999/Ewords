...
(function(){
  ...
  function atlasStyle(row,dir,extra=''){const col=ATLAS_COL[dir]??0,x=(col/3*100).toFixed(4),y=(row/5*100).toFixed(4);return `background-image:url(${atlasSrc});background-size:400% 600%;background-position:${x}% ${y}%;background-repeat:no-repeat;${extra}`}
  function spiderStyle(dir){const col=ATLAS_COL[dir]??0,x=(col/3*100).toFixed(4);return `background-image:url(${spiderAtlasSrc});background-size:400% 100%;background-position:${x}% 50%;background-repeat:no-repeat;`}
  ...
  function refresh(el){
    ...
    if(name==='毒蜘蛛'){
      if(spiderAtlasSrc){el.innerHTML=`<span class="enemyGlyph directionalEnemy enemyAtlas512 spiderRendered" style="${spiderStyle(dir)}"></span>${hp}`;return}
  ...
  async function loadSpider(){spiderAtlasSrc=await loadTextAsset(SPIDER_ATLAS_TEXT);document.querySelectorAll('.entity.enemy[title="毒蜘蛛"]').forEach(e=>delete e.dataset.fullSprite);refreshAll()}
  ...
  window.addEventListener('DOMContentLoaded',()=>{const b=document.getElementById('board');if(b)obs.observe(b,{childList:true,subtree:true,characterData:true});refreshAll();loadAtlas();loadSpider()});
})();