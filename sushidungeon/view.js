'use strict';
const BASE_GENERATE=generateFloor,BASE_ATTACK=attack,BASE_ENEMY_TURN=enemyTurn,BASE_MOVE=move;
...
move=function(dx,dy){
  ...
  BASE_MOVE(dx,dy);
  if(before&&game&&!game.dead){const moved=before.x!==game.player.x||before.y!==game.player.y;if(moved){playerMotion='walk';followCamera();game.enemies.forEach(e=>{const px=game.player.x-e.x,py=game.player.y-e.y;if(Math.abs(px)+Math.abs(py)<8)e.facing=facingFrom(px,py,e.facing||'s')});render();setTimeout(()=>{if(game&&!game.dead&&playerMotion==='walk'){playerMotion='idle';render()}},190)}}
  render();
};
