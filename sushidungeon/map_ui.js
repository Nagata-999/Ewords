...
(function(){
  ...
  function buildCell(x,y,seen,mode){
    ...
    const floor=!!game.grid[y]?.[x];
    const known=!!(game.mapReveal||!seen||seen.has(key(x,y)));
    const current=known&&isVisibleNow(x,y);
    c.className='mapCell '+(!known?'unknown':floor?'floor':'wall');
    if(known)c.classList.add(current?'current':'explored');

    if(mode==='mini'){
      // Minimap prioritizes exploration readability over terrain detail.
      if(!known)c.style.background='#020406';
      else if(!floor)c.style.background='#0a1319';
      else if(current)c.style.background='#32d7ff';
      else c.style.background='#ffd23f';
    }else{
  ...
  function paint(grid,mode){
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++)grid.append(buildCell(x,y,seen,mode));
  }
