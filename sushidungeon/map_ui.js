...
(function(){
  ...
  function buildCell(x,y,seen){
    ...
    if(known)c.classList.add(isVisibleNow(x,y)?'current':'explored');
    if(known&&floor){
  ...
  function paint(grid){if(!grid||!game||!game.grid)return;const h=game.grid.length,w=Math.max(...game.grid.map(r=>r.length)),seen=game.seen instanceof Set?game.seen:null;grid.style.gridTemplateColumns=`repeat(${w},1fr)`;grid.replaceChildren();for(let y=0;y<h;y++)for(let x=0;x<w;x++)grid.append(buildCell(x,y,seen))}
