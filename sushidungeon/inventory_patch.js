...
(function(){
  ...
  // Keep the English chest reward inside the same 20-slot rule.
  openWordChest=function(){
    ...
    $('wordPrompt').textContent=q.en;$('wordChoices').replaceChildren();
    for(const c of choices){
      const b=document.createElement('button');b.type='button';b.textContent=c;
      b.onclick=()=>{
        [...$('wordChoices').children].forEach(x=>x.disabled=true);
        if(c===q.jp){
          if(inventoryFull()){
            msg(`正解！ しかし持ち物がいっぱいだ。`);
            ...
            msg(`正解！ ${prize.icon} ${prize.name}を手に入れた。`);
            log(`英単語宝箱「${q.en}」正解 → ${prize.name}`);
          }
        }else{msg(`不正解。「${q.en}」＝ ${q.jp}`);log(`英単語宝箱「${q.en}」不正解`)}
        updateCount();
        setTimeout(()=>{$('wordDialog').close();endTurn()},650);
      };
      $('wordChoices').append(b);
    }
