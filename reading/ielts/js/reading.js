...
const passageId = params.get('id') || '1-1';
const storageKey = suffix => `ielts_${passageId}_${suffix}`;
...
function renderApp(){
  ...
  document.title=`${data.id} ${data.title} | IELTS Reading`;
  document.title+='｜すし単';
  // Keep each real passage distinct; ignore unrelated tracking/query parameters.
  let canonical=document.querySelector('link[rel="canonical"]');
  if(!canonical){canonical=document.createElement('link');canonical.rel='canonical';document.head.appendChild(canonical);}
  canonical.href='https://sushitan.net/reading/ielts/reading?id='+encodeURIComponent(data.id);
  let description=document.querySelector('meta[name="description"]');
  if(!description){description=document.createElement('meta');description.name='description';document.head.appendChild(description);}
  description.content=`IELTS Readingの練習問題 ${data.id}「${data.title}」。本文・設問・採点・解説を使って学習できます。すし単の無料英語教材です。`;
  ...
  $('#clearAnswersBtn').onclick=()=>{if(confirm('このPassageの解答と結果を削除しますか？')){['answers','flags','result','best','vocab'].forEach(k=>localStorage.removeItem(storageKey(k)));location.reload();}};
  ...
  $('#downloadSubmitBtn').onclick=()=>{const blob=new Blob([$('#submitText').value],{type:'text/plain;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`IELTS_${passageId}_${studentName||'student'}.txt`;a.click();URL.revokeObjectURL(a.href);};
  $('#saveNameBtn').onclick=()=>{studentName=$('#studentNameInput').value.trim();className=$('#classNameInput').value.trim();localStorage.setItem('ielts_student_name',studentName);localStorage.setItem('ielts_class_name',className);location.reload();};
