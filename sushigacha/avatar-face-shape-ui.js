...
(() => {
  ...
  const expressions=window.SushiAvatarFun?.expressions||['ふつう','にこにこ','ドヤ顔','びっくり','ねむそう'];
  ...
  function ensure(){const host=document.getElementById('customizer');if(!host)return;button(host,'faceShapePart','faceShape','輪郭',faceNames);button(host,'expressionPart','expression','表情',expressions);button(host,'petPart','pet','ペット',pets);button(host,'auraPart','aura','オーラ',auras);}
