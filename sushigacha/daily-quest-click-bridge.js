...
(()=>{
  ...
  if(key==='idiom'){const seen=new WeakSet();const scan=()=>{document.querySelectorAll('.good,.correct,.correct-flash,[data-correct="true"]').forEach(el=>{const target=el.closest?.('button,.choice')||el;if(!target||seen.has(target))return;seen.add(target);add()})};new MutationObserver(scan).observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','data-correct']});scan();return}
