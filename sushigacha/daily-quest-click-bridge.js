...
(()=>{
  ...
  if(key==='quiz'){let last='';const check=()=>{const el=document.getElementById('soloRes');if(!el)return;const t=(el.textContent||'').trim();if(t===last)return;last=t;if(/^✅\s*Correct!/i.test(t))add()};new MutationObserver(check).observe(document.documentElement,{subtree:true,childList:true,characterData:true,attributes:true});check();return}
