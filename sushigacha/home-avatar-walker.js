...
(() => {
  ...
  function grantLoginRewards(){
    ...
    if(!l.dailyGemClaims.includes(today)){
      l.gems=(Number.isSafeInteger(l.gems)?l.gems:0)+10;
      l.dailyGemClaims=[...l.dailyGemClaims,today].slice(-180);
      messages.push('毎日ボーナス +10💎');
    }
    const total=Number.isSafeInteger(l.total)?l.total:0;
    if(total>0&&total%10===0&&!l.loginAvatarMilestones.includes(total)){
      l.gacha=l.gacha&&typeof l.gacha==='object'?l.gacha:{};
      l.gacha.owned=Array.isArray(l.gacha.owned)?l.gacha.owned:[];
      const candidates=rewardIds.filter(id=>!l.gacha.owned.includes(id));
      if(candidates.length){const id=candidates[Math.floor(Math.random()*candidates.length)];l.gacha.owned.push(id);messages.push(`累計${total}日！アバターアイテムGET`);}else{l.gems=(Number.isSafeInteger(l.gems)?l.gems:0)+50;messages.push(`累計${total}日！コンプボーナス +50💎`);}
      l.loginAvatarMilestones.push(total);
    }
