...
(() => {
  ...
  /* Login stamp card is deliberately independent from avatar loading.
     Even if an avatar asset fails, login progress remains visible. */
  ...
  function renderStampCard(){
    ...
    const l=readLedger(),streak=Math.max(0,Number(l.streak)||0),total=Math.max(0,Number(l.total)||0),pos=streak?((streak-1)%3)+1:0;
    const remain=pos===0?3:3-pos,cycleStart=streak?streak-pos+1:1;
    const stamps=[1,2,3].map(i=>{const absolute=cycleStart+i-1,done=i<=pos,reward=i===3;return `<div class="login-stamp ${done?'done ':''}${i===pos?'today ':''}${reward?'reward':''}"><div class="stamp-icon">${done?'✅':reward?'💎':'🍣'}</div><div class="stamp-day">DAY ${absolute}${reward?' · 100💎':''}</div></div>`}).join('');
  ...
  // Render the login UI immediately. Do not wait for avatar libraries.
  installStampCard();
  window.addEventListener('pageshow',()=>{installStampCard();renderStampCard()});
  window.addEventListener('storage',e=>{if(e.key===LEDGER)renderStampCard()});
  ...
  char.addEventListener('click',()=>location.href='sushigacha/sushi-avatar.html');window.addEventListener('resize',()=>{x=Math.min(x,Math.max(18,innerWidth-char.offsetWidth-18))});window.addEventListener('focus',()=>{render();renderStampCard()});
